...
describe('GetValidServerRootForWorkspace', () => {
	...
	it('returns serverRoot if exists', async () => {
		sandbox.stub(PathUtil, 'FileExistsStat').returns(Promise.resolve({ exists: true, stat: Object.create(Stats.prototype) }));
	...
	it('returns no root if serverRoot does not exists', async () => {
		sandbox.stub(PathUtil, 'FileExistsStat').returns(Promise.resolve({ exists: false, stat: Object.create(Stats.prototype) }));
