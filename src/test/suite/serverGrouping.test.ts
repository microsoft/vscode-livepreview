...
describe('ServerGrouping', () => {
			...
			http.get(
				{
					host: '127.0.0.1',
					port: port,
					path: `/${openAtAddr === undefined ? '' : openAtAddr}index.html`,
				},
				function (res) {
					...
					res.on('end', function () {
						assert.equal(
							str,
							`<script type="text/javascript" src="${INJECTED_ENDPOINT_NAME}"></script>file contents from c:/Users/TestUser/workspace${workspaceNum}/test/index.html`
						);
						resolve();
					});
				}
			...
			http.get(
				{
					host: '127.0.0.1',
					port: port,
					path: `${openAtAddr === undefined ? '' : `/${openAtAddr}/`}`,
				},
				function (res) {
					assert.equal(res.statusCode, 200);
					res.on('data', function (data) {
			...
			http.get(
				{
					host: '127.0.0.1',
					port: port,
					path: `/${openAtAddr === undefined ? '' : openAtAddr}path/`,
				},
				function (res) {
					assert.equal(res.statusCode, 200);
					res.on('data', function (data) {
					...
					res.on('end', function () {
						assert(
							str.indexOf('Index of /path/') > -1 ||
								str.indexOf('Index of path') > -1
						);
						assert(
							str.indexOf('<th>Name</th><th>Size</th><th>Date Modified</th>') >
								-1
						);
			...
			http.get(
				{
					host: '127.0.0.1',
					port: port,
					path: `/${openAtAddr === undefined ? '' : openAtAddr}path/eep.html`,
				},
				function (res) {
			...
			http.get(
				...
				function (res) {
					...
					assert.equal(
						...
						openAtAddr === undefined
							? '/path/'
							: `/c:/Users/TestUser/workspace${workspaceNum}/test/path/`
					);
					resolve();
				}
			);
	...
	describe('With a workspace', () => {
		...
		before(async () => {
			...
			serverTaskProvider = new ServerTaskProvider(
				telemetryReporter,
				endpointManager,
				connectionManager
			);
	...
	describe('Without workspace', () => {
		...
		before(async () => {
			...
			openAtAddr = await endpointManager.encodeLooseFileEndpoint(
				'c:/Users/TestUser/workspace1/test'
			);
		});
		...
		it('should inform the user that there is no server root if they try to go to root', async () => {
			return new Promise<void>((resolve, reject) => {
				let str = '';
				http.get(
					{ host: '127.0.0.1', port: serverGrouping.port, path: `/` },
					function (res) {
						assert.equal(res.statusCode, 404);
						res.on('data', function (data) {
							str += data;
	...
	describe('Two workspaces', () => {
		...
		before(async () => {
			await init();
			connectionManager = new ConnectionManager();
			connection1 = await connectionManager.createAndAddNewConnection(
				testWorkspaces[0]
			);
			connection2 = await connectionManager.createAndAddNewConnection(
				testWorkspaces[1]
			);
			endpointManager = new EndpointManager();
			serverTaskProvider = new ServerTaskProvider(
